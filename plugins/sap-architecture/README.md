...
| `render.py <file>.drawio` | Render a `.drawio` file to PNG/SVG/PDF via the draw.io desktop CLI. Auto-discovers `$DRAWIO_CLI`, standard install paths, and a verified project-local `.cache/drawio-runtime/draw.io.app`. `--batch <dir>` renders every diagram in a folder. |

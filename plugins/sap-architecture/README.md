...
| `render_semantic.py "<request>" --out <file>` | Deterministic SAP-style fallback renderer for ceiling-limited prompts. Supports `security-operations`, `devops`, `on-prem-connectivity`, `private-connectivity`, `btp-application`, `integration-flow`, and `ai-agent`. |
